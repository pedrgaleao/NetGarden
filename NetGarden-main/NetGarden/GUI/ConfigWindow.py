...
from NetGarden.GUI.i18n import (
    LANG_EN,
    LANG_PT,
    load_settings,
    save_settings,
...
class ConfigWindow(QWidget):
    ...
    def apply_lang(self):
        ...
        self.client_port_label.setText(
            tr(
                self.lang,
                "client_port",
            )
        ...
        self.server_port_label.setText(
            tr(
                self.lang,
                "server_port",
            )
    ...
    def refresh_recents_ui(self):
        ...
        for recent in self.recents:
            self.recent_list.addItem(
                f"{recent['client_ip']}:"
                f"{recent['client_port']} → "
                f"{recent['server_ip']}:"
                f"{recent['server_port']}"
            )
    ...
    def pick_recent(self, item):
        ...
        if (
            index < 0
            or index >= len(self.recents)
        ):
            return

        recent = self.recents[index]
    ...
    def start_clicked(self):
        try:
            cfg = {
                "client_ip": (
                    self.client_ip.text().strip()
                ),
                "client_port": int(
                    self.client_port.text().strip()
                ),
                "server_ip": (
                    self.server_ip.text().strip()
                ),
                "server_port": int(
                    self.server_port.text().strip()
                ),
                "lang": self.lang,
            }
        ...
        self.recents = [
            ...
            if not (
                recent["client_ip"]
                == cfg["client_ip"]
                and recent["client_port"]
                == cfg["client_port"]
                and recent["server_ip"]
                == cfg["server_ip"]
                and recent["server_port"]
                == cfg["server_port"]
            )
        ]
